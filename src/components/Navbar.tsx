...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <header className="sticky top-0 z-50 w-full backdrop-blur-xl bg-[#EFEEE0]/90 border-b border-[#B0814D]/25 shadow-xs transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-16 sm:h-20 gap-4">
          ...
          <nav className="hidden md:flex items-center gap-1 lg:gap-2">
            {navLinks.map((link, idx) => (
              <a
                ...
                className="px-3.5 py-2 rounded-xl text-xs font-bold text-[#181816] hover:text-[#0C478D] hover:bg-[#B0814D]/10 transition whitespace-nowrap"
              >
                {link.name[lang]}
              </a>
            ))}
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <div className="flex items-center p-1 rounded-2xl bg-[#FAF9F3] border border-[#B0814D]/30 text-xs font-bold shadow-2xs">
              <button
                onClick={() => setLang('ca')}
            ...
            <button
              ...
              className="p-2 rounded-2xl md:hidden text-[#181816] hover:bg-[#B0814D]/15 border border-[#B0814D]/30 transition cursor-pointer"
              aria-label="Menú"
            >
      ...
      {/* Mobile Drawer Menu */}
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-[#B0814D]/25 bg-[#FAF9F3]/98 backdrop-blur-2xl px-4 py-6 space-y-2 shadow-xl animate-in slide-in-from-top-2">
          {navLinks.map((link, idx) => (
            <a
              key={idx}
              href={link.href}
